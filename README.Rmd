...
sequence ID (`SeqId`), which is in the format `####-##`. In this package, the 
...
gene-based annotations), and vice versa. For more information about `SeqIds`, 
see [?SomaDataIO::SeqId](https://somalogic.github.io/SomaDataIO/reference/SeqId.html)
...
SomaScan data, you will likely find it highly useful.
`SomaDataIO` is available on [CRAN](https://cran.r-project.org/package=SomaDataIO).
...
4. `mapIds` retrieves annotation data (from only a single data type, aka column):

```{r eval=FALSE}
mapIds(SomaScan.db, keys = "18342-2", columns = "SYMBOL", multiVals = "first")
```

5. `select` retrieves annotation data en masse using `keys` and `columns`:

```{r eval=FALSE}
select(SomaScan.db, keys = "18342-2", columns = c("ENTREZID", "SYMBOL"))
```

For more detailed usage examples, please see the package vignettes and the
[Bioconductor documentation](https://bioconductor.org/packages/release/bioc/html/AnnotationDbi.html).
...
entities not represented here. As such, this value may differ from the exact 
number in your ADAT. 
